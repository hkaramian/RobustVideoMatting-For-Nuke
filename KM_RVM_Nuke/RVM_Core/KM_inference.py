...
def convert_video(model,
    ...
    try:
        ...
        #     data = json.load(f)
        # f.close()
        # data["total_frames"] = str(len(source))
        # with open(json_file, "w") as file:
        #     json.dump(data, file)
        # file.close()
        # TCP socket to send progress 
        ...
        with torch.no_grad():
            ...
            for src in reader:
                ...
                # # KM
                progress_value = bar.n
                #print(f"KM Progress: {progress_value} out of {len(source)}")
                # sock.send(str(progress_value).encode('utf-8'))
                # # KM End
